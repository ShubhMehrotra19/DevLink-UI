...
import {
  ...
  const StaggeredDropDown = () => {
    ...
    return (
      <div className="p-3 flex items-center justify-center bg-transparent">
        <motion.div animate={open ? "open" : "closed"} className="relative top-0 left-0 z-30">
          <button
  ...
  const Option = ({ Icon }: any) => {
    return (
     <>
        <motion.span variants={actionIconVariants} className="flex justify-center items-center gap-2">
          <Image src={Icon} height={28} width={28} alt="" className=" hover:scale-105 active:scale-95 transition-all duration-300" />
        </motion.span>
     </>
    );
  };
